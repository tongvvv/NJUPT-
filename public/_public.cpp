...
int UTF8ToGBK(char* srcStr, char* desBuff, size_t desBuffLength)
{
  ...
  memset(desBuff,0,desBuffLength);
...
//itimeout=0 无限等待， 否则单位秒等待
bool Read(int m_connfd, char* buffer, const int itimeout )
{
...
bool Write(int m_connfd, const char* buffer, const int ibuflen )
{
        if (m_connfd == -1) return false;
