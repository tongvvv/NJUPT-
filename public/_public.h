...
///////////////////////////////////////////////////////////////////////////
// 以下是修改过的东西
// uft8和gbk编码之间的转换函数， desBuffLength为desBuff的大小
// 在函数体内会自动清空desBuff
int UTF8ToGBK(char* srcStr, char* desBuff, size_t desBuffLength);
int GBKToUTF8(char* srcStr, char* desBuff, size_t desBuffLength);

// 以下函数解决分包粘包问题！itimeout为等待时间 默认无限等待，否则单位为秒
// 下面只调用Read和Write就行了，函数默认Readbuffer大小合适
bool Read(int m_connfd, char* buffer, const int itimeout = 0);
bool Write(int m_connfd, const char* buffer, const int ibuflen = 0);
